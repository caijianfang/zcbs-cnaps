 ...
 * 
 */
package com.zcbspay.platform.cnaps.consumer;
 ...
 */
@Service
public class CNAPSPaymentConsumer implements ApplicationListener<ContextRefreshedEvent>{
	...
	@Autowired
	@Qualifier("cnapsPaymentListener")
	...
	public void startConsume() throws InterruptedException, MQClientException {
		 ...
		 */
		DefaultMQPushConsumer consumer = new DefaultMQPushConsumer(RESOURCE.getString("cnaps.payment.producer.group"));
		consumer.setConsumeFromWhere(ConsumeFromWhere.CONSUME_FROM_FIRST_OFFSET);
		consumer.setNamesrvAddr(RESOURCE.getString("single.namesrv.addr"));
		consumer.setInstanceName(RESOURCE.getString("cnaps.payment.instancename"));
		String subExpression = "";
		for(CNAPSPaymentTagsEnum tagsEnum:CNAPSPaymentTagsEnum.values()){
		...
		consumer.subscribe(RESOURCE.getString("cnaps.payment.subscribe"), subExpression);
		...
		consumer.start();
		log.info("{},CNAPSPaymentConsumer消费者启动",consumer.getInstanceName());
	}
	/**
	 *
	 * @param event
