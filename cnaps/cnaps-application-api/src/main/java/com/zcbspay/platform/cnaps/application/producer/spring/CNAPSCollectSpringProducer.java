 ...
 * @since 
 */
public class CNAPSCollectSpringProducer implements Producer{
	private final static Logger logger = LoggerFactory.getLogger(CNAPSCollectSpringProducer.class);
	...
	public void init() throws MQClientException{
		logger.info("【初始化CNAPSCollectSpringProducer】");
		if(StringUtils.isEmpty(namesrvAddr)){
			namesrvAddr = RESOURCE.getString("single.namesrv.addr");
		}
		logger.info("【namesrvAddr】"+namesrvAddr);          
		producer = new DefaultMQProducer(RESOURCE.getString("cnaps.collect.producer.group"));
		producer.setNamesrvAddr(namesrvAddr);
		Random random = new Random();
        producer.setInstanceName(RESOURCE.getString("cnaps.collect.instancename")+random.nextInt(9999));
        topic = RESOURCE.getString("cnaps.collect.subscribe");
        logger.info("【初始化CNAPSCollectSpringProducer结束】");
        producer.start();
	}
	...
	public SendResult sendJsonMessage(String message,Object tags) throws MQClientException,
		...
		CNAPSCollectTagsEnum withholdingTagsEnum = (CNAPSCollectTagsEnum) tags;
	...
	public ResultBean queryReturnResult(SendResult sendResult) {
		logger.info("【CNAPSCollectSpringProducer Callback receive Result message】{}",JSON.toJSONString(sendResult));
